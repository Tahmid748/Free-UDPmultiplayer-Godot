...
func interaction_execution(body) -> bool:
	if body is CharacterBody2D:
		if minecart.jumping_in_use: return false
		if !on_minecart:
			on_minecart = true
			...
			minecart.can_jump = true
			minecart.configure_action.rpc(true)
			minecart.prepare_minecart.rpc(true, false, MultiplayerManager.self_name, MultiplayerManager.self_color)
			...
			minecart.can_jump = false
			minecart.configure_action.rpc(false)
			minecart.prepare_minecart.rpc(false)
