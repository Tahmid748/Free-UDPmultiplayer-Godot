...
@export var surface_color : Color = Color(0.0, 0.592, 0.929, 0.776)
@export var water_fill_color : Color = Color(0.243, 0.463, 1.0, 0.6)
...
func _initiate_water() -> void:
	...
	new_area.visible = false #optional
	add_child(new_area)
...
func _process(delta: float) -> void:
	...
	for body in bodies_in_water:
		if body.is_in_group("player_body"):
			...
			if speed > 5.0: # Adjust this threshold to control sensitivity
				var splash_strength = clamp(speed * 0.06, 0.16, 16.0)
				splash(body.global_position, randf_range(-splash_strength, splash_strength))
				rpc("splash", body.global_position, randf_range(-splash_strength, splash_strength))
...
func _on_area_entered(area : Area2D) -> void:
	if area.is_in_group("player_torso"):
		var player = area.get_parent()
		player.in_water = true
		bodies_in_water.append(player)
		splash(player.global_position , -player.velocity.y * player_splash_multiplier)
		rpc("splash", player.global_position, -player.velocity.y * player_splash_multiplier)
